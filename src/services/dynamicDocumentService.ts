...
export async function crearRegistroDocumento(
  ...
  const payload = await applyAutogeneratedIdFields(schema, data);
...
export async function actualizarRegistroDocumento(
  ...
  const { firestore } = getCmsFirebaseServices();
  const payload = removeAutogeneratedIdFields(schema, data);
...
async function applyAutogeneratedIdFields(
  schema: CmsContentSchema,
  data: Record<string, unknown>
): Promise<Record<string, unknown>> {
  const idFields = getAutogeneratedIdFields(schema);
  if (!idFields.length) {
    return { ...data };
  }
...
function removeAutogeneratedIdFields(
  schema: CmsContentSchema,
  data: Record<string, unknown>
...
function getAutogeneratedIdFields(schema: CmsContentSchema): CmsFieldSchema[] {
  if (schema.storageType !== "document") {
    return [];
  }
  return schema.fields.filter((field) => field.type === "id");
}
