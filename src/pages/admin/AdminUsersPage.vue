...
<template>
 ...
 <h3 class="text-xl font-black ">Usuarios</h3>
 <button
 type="button"
 ...
 <p class="mt-1 text-sm ">Listado de usuarios con último login y control de rol.</p>

 <p v-if="!puedeEditarRoles" class="mt-3 rounded-md border px-3 py-2 text-sm ">
 Solo usuarios con rol admin pueden editar roles.
 </p>

 <p v-if="error" class="mt-3 rounded-md border px-3 py-2 text-sm ">
 {{ error }}
 </p>
 <p
 v-if="mensaje"
 ...
 <p v-if="cargando" class="mt-4 text-sm ">Cargando usuarios...</p>
 <p v-else-if="users.length === 0" class="mt-4 text-sm ">No hay usuarios registrados.</p>
 ...
 <div>
 <p class="text-sm font-semibold ">{{ user.email || user.id }}</p>
 <p class="text-xs ">UID: {{ user.id }}</p>
 <p class="text-xs ">Último login: {{ formatDate(user.lastLoginAt) }}</p>
 <p class="text-xs ">Rol actual: {{ roleLabel(user.role) }}</p>
 </div>
