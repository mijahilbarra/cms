...
if (schema.storageType === "document") {
  await applyAutogeneratedIdFields(db, schema, payload);
}
...
async function applyAutogeneratedIdFields(dbRef, schema, payload) {
  const idFields = Array.isArray(schema.fields)
    ? schema.fields.filter((field) => field && field.type === "id")
    : [];
  if (!idFields.length) {
    return;
  }
